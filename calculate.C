...
void calculate(TString inputFile) {
  TFile *f = new TFile(inputFile,"update");
  TTree *T = (TTree*)f->Get("ntuple");
  Int_t mass;
  Int_t charge;
  Float_t spin;
  float efficiency, uncertainty;
  double observedLimit;
  TBranch *limit = T->Branch("limit",&observedLimit);
  T->SetBranchAddress("mass", &mass);
  T->SetBranchAddress("charge", &charge);
  T->SetBranchAddress("spin", &spin);
  T->SetBranchAddress("efficiency",&efficiency);
  T->SetBranchAddress("uncertainty",&uncertainty);

  double ilum = 4000.0; // integrated luminosity in ipb
  double slum = 0.01*ilum; // uncertainty in the luminosity, here it is 1%
  double bck = 0.001;
  double sbck = 0.001;
  int n = 0; 
  int nuisanceModel = 1;
  ...
  for (Long64_t i=0; i < T->GetEntries(); i++) {
    T->GetEntry(i);
    observedLimit = -999.;
    if(efficiency < 0.001 || uncertainty > 1.1 * efficiency){
      cout<<"Dropping mass = "<<mass<<" GeV, charge = "<<charge<<" gD, spin = "<<spin<<" from limit calculation"<<endl;
    }else{
      observedLimit = CL95(ilum, slum, efficiency, uncertainty, bck, sbck, n, gauss, nuisanceModel); 
    }
    limit->Fill();
  }
  T->Write();
